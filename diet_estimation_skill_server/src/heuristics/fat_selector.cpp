...
namespace diet_estimation_skill {
    ...
    bool FatSelector::run(){
        ...
        for (it = candidate_list_.begin(); it != candidate_list_.end(); ++it)
        {
            ...
            if( !std::get<1>(*it) ) {
                double fat_value = std::get<0>(*it).fat_rate * std::get<0>(*it).gram;
                fat_value_arr_.push_back(fat_value);
                if (fat_value > threshold_) { 
        ...
        {
            ROS_DEBUG_STREAM("Max == Min. All candidates are eligible (before extrapolation analysis) according to the fat selector array.");

...
namespace diet_estimation_skill {
    ...
    void CaloricSelector::setupMethodConfigurationFromParameterServer(ros::NodeHandlePtr &_node_handle,ros::NodeHandlePtr &_private_node_handle, std::string _configuration_namespace){
        ...
        _private_node_handle->param<double>(_configuration_namespace + "fiber_cal_per_gram", fiber_cal_per_gram_, 2.0);
    ...
    bool CaloricSelector::run(){
        ...
        for (it = candidate_list_.begin(); it != candidate_list_.end(); ++it)
        {
            ...
            if( !std::get<1>(*it) ) {
                double protein_gram = std::get<0>(*it).protein_rate * std::get<0>(*it).gram;
                double fat_gram = std::get<0>(*it).fat_rate * std::get<0>(*it).gram;
                double fiber_gram = std::get<0>(*it).fiber_rate * std::get<0>(*it).gram;
                double caloric_value = (protein_gram * protein_cal_per_gram_) +
                                        (fat_gram * fat_cal_per_gram_) +
                                        (fiber_gram * fiber_cal_per_gram_);
                caloric_value_arr_.push_back(caloric_value);
                if (caloric_value > threshold_) { 
        ...
        {
            ROS_DEBUG_STREAM("Max == Min. All candidates are eligible (before extrapolation analysis) according to the caloric selector array.");
